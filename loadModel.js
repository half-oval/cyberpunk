...
addedModel = { "0": { "id": "chair", "position": { "x": 1, "y": 0, "z": -1 }, "rotation": { "_x": 0, "_y": 0, "_z": 0, "_order": "XYZ" } }, "1": { "id": "chair", "position": { "x": 1, "y": 0, "z": -2 }, "rotation": { "_x": 0, "_y": 0, "_z": 0, "_order": "XYZ" } } }

// console.log(Object.keys(addedModel).length)

// var remainingUrls = []
// for (var j = 0; j < Object.keys(addedModel).length; j++) {
//     remainingUrls.push('chair.glb')
// }
...
function init() {
    ...
    //     loader.load("load.json", function (data) {
    //         console.log(scene)
    //         scene = data;
    //     },
    //     function (xhr) {
    ...
    new RGBELoader()
        ...
        .load('venice_sunset_1k.hdr', function (texture) {
            ...
            //JSON Room Model
            // var jloader = new THREE.ObjectLoader().setPath('models/');
            // jloader.load("microplane.json", function (scene) {
            //     group.add(scene);
            //     //scene.add(scene)
            // },
            // function (xhr) {
            ...
            // loader.load('bomb.glb', function (gltf) {
            ...
            if (addedModel) {
                ...
                // var nloader = new THREE.GLTFLoader().setPath('models/');
                // for (let i = 0; i < resourceData.length; i++) {
                //     let oResource = resourceData[i];
                //     let sModelName = "resources/" + oResource.model3D + ".gltf";
                //     nloader.load(sModeName, function (gltf) {
                ...
                const nloader = new GLTFLoader().setPath('models/');
                for (var key in addedModel) {
                    let data = addedModel[key] 
                    console.log(data)
                    let modelName = addedModel[key].id +".glb";
                    nloader.load(modelName, function (gltf) {
                        console.log(gltf.scene)
                        gltf.scene.position.set(data.position.x,data.position.y,data.position.z)
                        gltf.scene.rotation.set(data.rotation._x,data.rotation._y,data.rotation._z)
