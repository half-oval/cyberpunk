...
function onLeftSqueezeStart(event){
    ...
    var position = scene.children[1].children.forEach(element => {
        console.log(element);
        var elementDta = {id: element.userData,position: element.position, rotation: element.rotation};
        obj[i] = elementDta;
        i++;
    });;

    //var sceneData = scene.children[0].toJSON();
    console.log(obj)
    

    download('data.json', obj);
}
